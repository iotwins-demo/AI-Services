...
def _save_trained_model(model_name, model, stats, scaler):
    ...
    ae_model_name = '{}{}'.format(out_dir, model_name)
    ...
    # save scaler
    ae_scaler_file = '{}{}_scaler.save'.format(out_dir, 
            model_name)
    joblib.dump(scaler, ae_scaler_file) 

    return ae_model_name, ae_stats_file, ae_stats_file
...
def _load_trained_model(model_name):
    ...
    model_name = '{}{}'.format(out_dir, model_name)

    # Keras model -- the trained autoencoder
    model_name_ext = '{}.h5'.format(model_name)
    # check if the trained model exists
    if os.path.isfile(model_name_ext):  
        model = load_model(model_name_ext)
    else:
    ...
    if os.path.isfile(stats_file):  
        gs.load_py_obj(stats_file)
    else:
        print("[adssae:load_trained_model] No stats file with name {}".format(
            stats_file))
        stats = {}

    scaler_file = '{}{}_scaler.save'.format(out_dir, model_name)
...
def semisup_autoencoder(df_fname, sep=',', user_id='default', task_id='0.0', 
    ...
    # read hyperparameters from binary file (pickle object)
        ...
        hparams = load_py_obj(hparams_file)
    ...
    if n_percentile == -1:
        ...
        acc_stats = ae.detection_withThreshold(percentile, test_noAnomalies,
...
def sup_autoencoder_classr(df_fname, sep=',', user_id='default', task_id='0.0', 
    ...
    # read hyperparameters for autoencoder from binary file (pickle object)
        ...
        hparams_ae = load_py_obj(hparams_file_ae)
    ...
    # read hyperparameters for classifier from binary file (pickle object)
        ...
        hparams_classr = load_py_obj(hparams_file_classr)
