...
def get_categorical_continuous_features(df_fname):
    ...
    '''
    df_fname_ext = '{}/{}'.format(volume_dir, df_fname)
    df = pd.read_csv(df_fname_ext)
    cat_feats, cont_feats = _get_categorical_continuous_features(df)
    # save the results 
    cat_feats_fname = '{}/categorical_feats.pickle'.format(out_dir)
    serialize_py_obj(categorical_features, cat_feats_fname)
    cont_feats_fname = '{}/continuous_feats.pickle'.format(out_dir)
    serialize_py_obj(continuous_features, cont_feats_fname)
    return categorical_features, continuous_features
...
def encode_categorical_feature(df_fname, cat_feat):
    ...
    '''
    df_fname_ext = '{}/{}'.format(volume_dir, df_fname)
    df_in = pd.read_csv(df_fname_ext)
    df = _encode_categorical_feature(df_in)
    # save the results 
    df_fname_encoded = '{}/{}_enc.csv'.format(out_dir, df_fname.split('.')[0])
    df.to_csv(df_fname_encoded)
    return df
...
def normalize(df_fname, cat_feats=[], cont_feats=[], scaler_in=None,
    ...
    df, scaler = _normalize(df, cat_feats=[], cont_feats=[], scaler_in=None,
            scalerType='minmax')
    # save the results 
    df_fname_norm = '{}/{}_norm.csv'.format(out_dir, df_fname.split('.')[0])
    df.to_csv(df_fname_norm)
    scaler_fname = '{}/scaler.pickle'.format(out_dir)
...
def preprocess(df_fname, y='', scaler_in=None):
    ...
    df_fname_norm = '{}/{}_norm.csv'.format(out_dir, df_fname.split('.')[0])
    df.to_csv(df_fname_norm)
    norm_target_fname = '{}/normalized_label.pickle'.format(out_dir)
...
def split_data_withLabel(df_fname, y, ratio = 0.7):
    ...
    y_test_fname = '{}/{}_y_test.pickle'.format(out_dir, 
            df_fname.split('.')[0])
    serialize_py_obj(y_test, y_test_fname)
    y_val_fname = '{}/{}_y_val.pickle'.format(out_dir, df_fname.split('.')[0])
    serialize_py_obj(y_val, y_val_fname)
    return df_train, df_test, df_val, y_train, y_test, y_val
...
def oversample(X_fname, y_fname, method='SMOTE', strat='not majority'):
    ...
    # save the results 
    X_resampled_fname = '{}/X_resampled.pickle'.format(out_dir)
    serialize_py_obj(X_resampled, X_resampled_fname)
    y_resampled_fname = '{}/y_resampled.pickle'.format(out_dir)
    serialize_py_obj(y_resampled, y_resampled_fname)
    return X_resampled, y_resampled
...
def sanitize(input_str):
    ...
    # save the results 
    out_str_fname = '{}/sanitized_str.txt'.format(out_dir)
    with open(out_str_fname, "w") as txt_file:
            txt_file.write('{}'.format(out_str))
    return out_str
